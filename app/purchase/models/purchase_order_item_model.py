...
from inventory.models import EnologicalMaterialModel,PackagingMaterialModel,LabelMaterialModel
...
class PurchaseOrderItem(models.Model):
    ...
    unit_price = models.DecimalField(max_digits=10, decimal_places=4,validators=[MinValueValidator(Decimal(0.0001))],verbose_name='precio unitario')

    def clean(self):
        if hasattr(self, 'purchase_order') and self.purchase_order:
            if self.purchase_order.status == PurchaseOrder.Status.CLOSED:
                raise ValidationError("No se puede modificar una línea de una orden cerrada.")
        ...
        if count == 0:
            raise ValidationError("Debe seleccionar un producto (Packaging, Etiqueta o Enológico).")
        if count > 1:
            raise ValidationError("Una línea de pedido solo puede contener un tipo de producto.")
    
    def delete(self, *args, **kwargs):
        # BLOQUEO DE ELIMINACIÓN DE LÍNEA SI LA ORDEN ESTÁ CERRADA
        if self.purchase_order.status == PurchaseOrder.Status.CLOSED:
            raise ValidationError("No se puede eliminar una línea de una orden cerrada.")
