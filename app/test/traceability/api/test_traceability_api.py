...
class TestLotTraceabilityViewSet:
    ...
    def test_retrieve_by_lot_number_path(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
        ...
        url = reverse(
            "traceability:lot-traceability-detail", kwargs={"lot_number": lot}
        )
    ...
    def test_list_traceability_records(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
    ...
    def test_filter_by_wine_name_in_snapshot(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
        ...
        assert all(
            wine_name in r["content"]["order_details"]["wine_name"]
            for r in response.data
        )

    def test_search_filter_global(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
    ...
    def test_retrieve_non_existent_lot(self, api_client, user_factory):
        ...
        url = reverse(
            "traceability:lot-traceability-detail",
            kwargs={"lot_number": "LOTE-FANTASMA"},
        )
        response = api_client.get(url)
    ...
    def test_readonly_methods_only(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
        """
        ...
        assert (
            api_client.post(
                reverse("traceability:lot-traceability-list"), {}
            ).status_code
            == status.HTTP_405_METHOD_NOT_ALLOWED
        )

    def test_permission_denied_for_wrong_role(
        self, api_client, user_factory, orden_trazada_y_confirmada
    ):
