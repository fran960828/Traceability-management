...
class TestPurchaseOrderItemModel:
    ...
    # --- HAPPY PATH ---
    def test_create_valid_packaging_item(
        self, purchase_order_factory, packaging_factory
    ):
    ...
    def test_error_multiple_materials_exclusive(
        self, purchase_order_item_factory, packaging_factory, label_factory
    ):
        """VULNERABILIDAD: No permitir asignar dos tipos de materiales a una línea"""
        item = purchase_order_item_factory.build(
            packaging=packaging_factory(), label=label_factory(), enological=None
        )
        with pytest.raises(ValidationError) as exc:
            item.full_clean()
        assert "Una línea de pedido solo puede contener un tipo de producto" in str(
            exc.value
        )
    ...
    def test_error_no_material_selected(self, purchase_order_item_factory):
        ...
        item = purchase_order_item_factory.build(
            packaging=None, label=None, enological=None
        )
        with pytest.raises(ValidationError) as exc:
            item.full_clean()
    ...
    def test_negative_price_protection(
        self, purchase_order_item_factory, enological_factory
    ):
        """VULNERABILIDAD: Evitar precios negativos que alteren la valoración de stock"""
        item = purchase_order_item_factory.build(
            enological=enological_factory(), unit_price=-10.0
        )
        ...
        with pytest.raises(ValidationError) as excinfo:
            ...
            # 4. Asserts críticos
            assert (
                "unit_price" in errors
            ), "El error debería estar asociado al campo unit_price"
            # Opcional: Verificar que el mensaje es el del validador de precio
            assert any(
                "mayor que" in msg or "0.0001" in msg for msg in errors["unit_price"]
            )

    # --- PROTECCIÓN DE DATOS CERRADOS ---
    def test_prevent_item_modification_on_closed_order(
        self, purchase_order_factory, purchase_order_item_factory
    ):
