...
class StockMovementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def transfer_stock(self, request):
        ...
        if not all([origin_id, dest_id, batch_id, qty]):
            return Response(
                {"detail": "Faltan datos para la transferencia."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            with transaction.atomic():
                ...
                StockMovement.objects.create(
                    ...
                    movement_type="OUT",
                    user=request.user,
                    notes=f"Transferencia a ubicación {dest_id}",
                )
                StockMovement.objects.create(
                    batch_id=batch_id,
                    location_id=dest_id,
                    ...
                    movement_type="IN",
                    user=request.user,
                    notes=f"Transferencia desde ubicación {origin_id}",
                )
        ...
        return Response(
            {"detail": "Transferencia completada."}, status=status.HTTP_201_CREATED
        )
    ...
    def stock_adjustment(self, request):
        ...
        if serializer.is_valid():
            serializer.save(
                user=request.user, movement_type=StockMovement.MovementType.ADJUSTMENT
            )
