    ...
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="lot_number",
                type=str,
                description="Buscar por número de lote exacto",
            ),
            OpenApiParameter(
                name="wine_name",
                type=str,
                description="Filtrar por nombre del vino en el snapshot",
            ),
        ],
    ),
...
class LotTraceabilityViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = LotTraceability.objects.select_related(
        "production_order", "production_order__wine"
    ).all()
    ...
    search_fields = [
        ...
        "history_snapshot__order_details__wine_name",
    ]
    ...
    def download_pdf(self, request, lot_number=None):
        ...
        try:
            ...
            filename = (
                f"Informe_Trazabilidad_{instance.production_order.lot_number}.pdf"
            )
    ...
    def get_queryset(self):
        ...
        if wine_name:
            ...
            queryset = queryset.filter(
                history_snapshot__order_details__wine_name__icontains=wine_name
            )
